...
import { fetchDownloadStats, fetchDownloadLeads } from '../components/community/api';
...
const AdminDownloadsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const [dashStats, allLeads] = await Promise.all([
          fetchDownloadStats(),
          fetchDownloadLeads()
        ]);
        setStats(dashStats);
  ...
  // Filter and paginate locally since backend returns all currently
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              ...
              <KPICard icon={<Calendar size={20} />} label="Leads Today" value={stats?.leads_today ?? 0} />
            </div>
            ...
            <div className="bg-white/5 rounded-xl border border-white/10 overflow-hidden">
              <div className="p-6 border-b border-white/10">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <h2 className="text-white font-semibold">Full Leads List</h2>
