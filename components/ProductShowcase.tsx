...
const ProductShowcase: React.FC = () => {
  ...
  useEffect(() => {
    if (activeTab.images.length <= 1) return;

    const interval = setInterval(() => {
      setCurrentImageIndex(prev => (prev + 1) % activeTab.images.length);
    }, 3000);
              ...
              {activeTab.images.length > 1 && (
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20">
                  {activeTab.images.map((_, idx) => (
                    <div
                      key={idx}
                      className={`w-2 h-2 rounded-full transition-all ${idx === currentImageIndex ? 'bg-odara-primary w-4' : 'bg-white/20'}`}
                    />
                  ))}
                </div>
              )}
