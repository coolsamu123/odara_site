...
const AIDemo: React.FC = () => {
  ...
  const handleExampleChange = (ex: typeof activeExample) => {
    setActiveExample(ex);
    setView('pipeline');
  };

  const hasCodeView = !!activeExample.codeScreenshot;
          ...
          <div className="lg:col-span-8">
            <div className="rounded-xl overflow-hidden border border-white/10 shadow-2xl bg-[#0d1117]">
                ...
                <div className="flex items-center justify-between px-4 py-3 bg-white/5 border-b border-white/5">
                    <div className="flex gap-1">
                        <button
                          ...
                          className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-md transition-colors ${
                            view === 'pipeline'
                              ? 'bg-odara-primary/20 text-odara-primary'
                    ...
                    <div className="text-xs font-mono text-odara-muted">
                      {activeExample.label}
                    </div>
                </div>
